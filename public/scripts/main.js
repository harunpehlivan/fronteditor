...
toggle.addEventListener("click", function () {
    ...
    inner(toggleBool ? "menu" : "close")
    function inner(what) {
        document.querySelector(".material-symbols-outlined").innerHTML = what
    }
    document.querySelector(".resp").classList.toggle("active")
})

let doc = localStorage.getItem("doc")
if (doc == undefined) {
    projects.innerHTML = "You have no previous projects!"
}
else {
    ...
    docobj.forEach(function (element, index) {
        if (index > 2) {
            ...
            html += `<a href="/edit?project=${routepath}" target="_blank"><div class="hover_bg-black-gradient project">${element.project}</div></a>`
        }


    })
    html += `<div><a href="/projects" class="viewall">View all projects</a></div>`
...
setInterval(()=>{
    ...
    let onemain = document.querySelector(".onemain")
    let innerHtml = [ "Build SPAs Easily With", "Build Great UIs With", "Build Great Web Apps With", "Build Great Web Interfaces With","Show Your Creativity With"]
    let oneMainInnerHTML = [ `<a href="/templates#reacttemp" class="darkBlue">React</a>`, `<a href="/templates#tailwindtemp" class="blue">Tailwind CSS</a>`, `<a href="/templates#angulartemp" class="red">Angular.js</a>`, `<a href="/templates#vuetemp" class="green">Vue.js</a>`,`<a href="/templates#vanillatemp" class="white">Vanilla HTML, CSS, Js</a>`]
    num+=1;
    one.innerHTML = innerHtml[num-1]
    onemain.innerHTML = oneMainInnerHTML[num-1]
    if (num==5){
        num=0;
    }
